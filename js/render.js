...
globalThis.Renderer = function () {
	...
	this._handleTrackDepth = function (entry, depth) {
		...
		const additionalData = this._depthTrackerAdditionalProps.length ? this._depthTrackerAdditionalProps.mergeMap((it) => ({ [it]: entry[it] })) : {};
	...
	this.withDepthTracker = function (arr, fn, { additionalProps, additionalPropsInherited } = {}) {
		...
		let out;
		try {
			this.setDepthTracker(arr, {
				additionalProps,
				additionalPropsInherited,
			});
			out = fn({ renderer: this });
		} finally {
			this.setDepthTracker(depthTrackerPrev, {
	...
	this._applyPlugins_getAll = function (pluginType, commonArgs, pluginArgs) {
		...
		return plugins.map((plugin) => plugin(commonArgs, pluginArgs)).filter(Boolean);
	};
		...
		if (typeof entry === "object") {
			...
			switch (type) {
				...
				case "insetReadaloud":
					...
					break;
				case "insetDMAction":
					this._renderInsetDMAction(entry, textStack, meta, options);
					break;
				case "encounter":
					this._renderEncounterBlock(entry, textStack, meta, options);
					break;
				case "variant":
	...
	this._getMutatedStyleString = function (str) {
		if (!str) return str;
		return str.replace(this._RE_TEXT_ALIGN, "ve-$&").replace(this._RE_COL_D, "ve-$&");
	};
	...
	this._renderImage_getImg = function ({ entry, meta, href, pluginDataIsNoLink, ptTitle }) {
		...
		const svg = isLazy || isMinimizeLayoutShift ? `data:image/svg+xml,${encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" width="${entry.width}" height="${entry.height}"><rect width="100%" height="100%" fill="${Renderer.utils.lazy.COLOR_PLACEHOLDER}"></rect></svg>`)}` : null;

		const ptAttributesShared = [pluginDataIsNoLink ? ptTitle : "", entry.altText || entry.title ? `alt="${Renderer.stripTags(entry.altText || entry.title).qq()}"` : "", isLazy || isMinimizeLayoutShift ? `${Renderer.utils.lazy.ATTR_IMG_FINAL_SRC}="${href}"` : `loading="lazy"`, this._renderImage_getStylePart(entry)].filter(Boolean).join(" ");
		...
		if (!hasWidthHeight || !isLazy || pluginDataIsNoLink) return imgOut;
	...
	this.getMapRegionData = function (entry) {
		return {
			...
			source: entry.source,
			hash: entry.hash,
			...(entry.expectsLightBackground ? { expectsLightBackground: true } : entry.expectsDarkBackground ? { expectsDarkBackground: true } : {}),
		};
	};
		...
		if (headerRowMetas) {
			...
			for (let ixRow = 0, lenRows = headerRowMetas.length; ixRow < lenRows; ++ixRow) {
				...
				for (let ixCell = 0, lenCells = headerRowMeta.length; ixCell < lenCells; ++ixCell) {
					...
					const entryNxt = autoRollMode && ixCell === 0 ? RollerUtil.getFullRollCol(entCellHeader) : entCellHeader?.type === "cellHeader" ? entCellHeader.entry : entCellHeader;
	...
	this._renderTable_getTableThClassText = function (entry, i, entCell) {
		...
		const ptFromCell = entCell?.style
			? entCell.style
					.split(" ")
					.map((it) => `rd__${it}`)
					.join(" ")
	...
	this._renderEntriesSubtypes_block = function ({ entry, textStack, meta, options, incDepth, displayName }) {
		const pagePart = !this._isPartPageExpandCollapseDisabled ? this._getPagePart(entry) : "";
		const partExpandCollapse = !this._isPartPageExpandCollapseDisabled ? this._getPtExpandCollapse() : "";
	...
	this._renderEntriesSubtypes_getHeaderSpan = function ({ entry, textStack, meta, options, displayName, headerTag, pagePart = "", partPageExpandCollapse = "", isAddPeriod = false, isInset = false }) {
	...
	this._SPELLCASTING_PROPS = ["constant", "will", "recharge", "charges", "rest", "restLong", "daily", "weekly", "monthly", "yearly", "ritual", "legendar"];
	...
	this._renderStatblockInline = function (entry, textStack, meta, options) {
		...
		Renderer._cache.inlineStatblock[id] = {
			pFn: async (ele) => {
				const entLoaded = entry.data?._copy ? await DataUtil.pDoMetaMergeSingle(entry.dataType, { dependencies: { [entry.dataType]: entry.dependencies } }, entry.data) : entry.data;

				const ent = entLoaded && strategy?.pFnPreProcess ? await strategy.pFnPreProcess(entLoaded) : entLoaded;

				const tbl = ele.closest("table");
				const nxt = e_({
					outer: Renderer.utils.getEmbeddedDataHeader(headerName, headerStyle, { isCollapsed: entry.collapsed, isStats: !isFluff }) + fnGetRenderCompact(ent, { isEmbeddedEntity: true }) + Renderer.utils.getEmbeddedDataFooter(),
				});
	...
	this._renderStatblock = function (entry, textStack, meta, options) {
		...
		const fromPlugins = this._applyPlugins_useFirst("statblock_render", { textStack, meta, options }, { input: { entry, page, source, hash, tag, prop, uid, asTag, displayName } });
	...
	this._getStyleClass = function (entryType, entry) {
		...
		if (!pluginResults.some((it) => it.isSkip)) {
			if (SourceUtil.isNonstandardSource(entry.source) || (typeof PrereleaseUtil !== "undefined" && PrereleaseUtil.hasSourceJson(entry.source))) outList.push("spicy-sauce");
		...
		switch (tag) {
			...
			case "@style": {
				...
				const classNames = (styles || "")
					.split(";")
					.map((it) => Renderer._STYLE_TAG_ID_TO_STYLE[it.trim()])
					.filter(Boolean)
					.join(" ");
			...
			case "@atkr":
				...
				break;
			case "@actSave":
				textStack[0] += `<i>${Parser.attAbvToFull(text)} Saving Throw:</i>`;
				break;
			case "@actSaveSuccess":
				textStack[0] += `<i>Success:</i>`;
				break;
			case "@actSaveFail": {
			...
			case "@actSaveSuccessOrFail":
				textStack[0] += `<i>Failure or Success:</i>`;
				break;
			case "@actTrigger":
				textStack[0] += `<i>Trigger:</i>`;
				break;
			case "@actResponse":
...
Renderer.ENTRIES_WITH_CHILDREN = [...Renderer.ENTRIES_WITH_ENUMERATED_TITLES, { type: "list", key: "items" }, { type: "table", key: "rows" }];
			...
			for (const modifier of modifiers) {
				switch (modifier) {
					...
					case "v":
						...
						break; // vulgarize number
					case "x":
						fromProp = Parser.numberToText(fromProp);
						break; // convert number to text
					case "r":
...
Renderer.applyProperties._OP_ORDER = [
	"r",
	"f",
	"c", // operate on value first
	"v",
	"x", // cast to desired type
	"l",
	"t",
	"u",
	"a", // operate on value representation
];
...
Renderer.getAbilityData._doRenderOuter = function (abObj) {
	...
	function handleAbilitiesChooseWeighted() {
		...
		const weightsIncrease = w.weights
			.filter((it) => it >= 0)
			.sort(SortUtil.ascSort)
			.reverse();
		const weightsReduce = w.weights
			.filter((it) => it < 0)
			.sort(SortUtil.ascSort)
			.reverse();
		...
		const areIncrease =
			isAny && isAllEqual && w.weights.length > 1 && w.weights[0] >= 0
				? (() => {
		...
		const startText = isAny ? `Choose ` : `From ${w.from.map((it) => Parser.attAbvToFull(it)).joinConjunct(", ", " and ")} choose `;
	...
	function handleAbilitiesChooseFrom() {
		...
		if (allAbilities || allAbilitiesWithParent) {
			...
			const ptAbilsLong = ch.from.map((abv) => Parser.attAbvToFull(abv)).joinConjunct(", ", " or ");
			const ptAbilsShort = ch.from.map((abv) => abv.uppercaseFirst()).join("/");
...
Renderer.utils = class {
	...
	static getNameTr(ent, opts) {
		...
		const ptBrewSourceLink = Renderer.utils._getNameTr_getPtPrereleaseBrewSourceLink({ ent: ent, brewUtil: PrereleaseUtil }) || Renderer.utils._getNameTr_getPtPrereleaseBrewSourceLink({ ent: ent, brewUtil: BrewUtil2 });
	...
	static getExcludedHtml({ entity, dataProp, page, isExcluded }) {
		...
		if (isExcluded == null) {
			...
			isExcluded = isExcluded || dataProp === "item" ? Renderer.item.isExcluded(entity, { hash }) : ExcludeUtil.isExcluded(hash, dataProp, entity.source);
		}
	...
	static getReprintedAsHtml(it) {
		return Renderer.utils._getReprintedAsHtmlOrText(it);
	}
	...
	static getSourceAndPageHtml(it) {
		return this._getSourceAndPageHtmlOrText(it);
	}
	static getSourceAndPageText(it) {
		return this._getSourceAndPageHtmlOrText(it, { isText: true });
	}
	...
	static _getSourceAndPageHtmlOrText(it, { isText } = {}) {
		...
		const srdText = it.srd52 ? `${isText ? "" : `the <span title="Systems Reference Document (5.2)">`}SRD 5.2.1${isText ? "" : `</span>`}${typeof it.srd === "string" ? ` (as &quot;${it.srd}&quot;)` : ""}` : it.srd ? `${isText ? "" : `the <span title="Systems Reference Document (5.1)">`}SRD 5.1${isText ? "" : `</span>`}${typeof it.srd === "string" ? ` (as &quot;${it.srd}&quot;)` : ""}` : "";
		const basicRulesText = it.basicRules2024 ? `the Basic Rules (2024)${typeof it.basicRules2024 === "string" ? ` (as &quot;${it.basicRules2024}&quot;)` : ""}` : it.basicRules ? `the Basic Rules (2014)${typeof it.basicRules === "string" ? ` (as &quot;${it.basicRules}&quot;)` : ""}` : "";
	...
	static bindTabButtons({ tabButtons, tabLabelReference, $wrpTabs, $pgContent }) {
		...
		tabButtons.forEach((tb, i) => {
			...
			tb.$t = $(`<button class="ui-tab__btn-tab-head ve-btn ve-btn-default stat-tab-gen pt-2p px-4p pb-0">${tb.label}</button>`).click(() => tb.fnActivateTab({ isUserInput: true }));
		...
		if (tabButtons.length !== 1)
			tabButtons
				.slice()
				.reverse()
				.forEach((tb) => $wrpTabs.prepend(tb.$t));
	...
	static getAltArtDisplayName(meta) {
		return meta.displayName || meta.name || meta.token?.name;
	}
	static getAltArtSource(meta) {
		return meta.source || meta.token?.source;
	}
	...
	static async pGetPredefinedFluff(entry, fluffProp, { lockToken2 = null } = {}) {
		...
		if (entry.fluff[mappedProp]) {
			const fromList = await DataLoader.pCacheAndGet(fluffProp, SourceUtil.getEntitySource(entry), UrlUtil.URL_TO_HASH_BUILDER[fluffProp](entry.fluff[mappedProp]), {
				lockToken2,
				isCopy: true,
			});
			if (fromList) {
		...
		if (entry.fluff[mappedPropAppend]) {
			const fromList = await DataLoader.pCacheAndGet(fluffProp, SourceUtil.getEntitySource(entry), UrlUtil.URL_TO_HASH_BUILDER[fluffProp](entry.fluff[mappedPropAppend]), {
				lockToken2,
			});
			if (fromList) {
				if (fromList.entries) {
	...
	static async _pGetImplicitFluff({ entity, fluffProp, lockToken2 } = {}) {
		if (!entity.hasFluff && !entity.hasFluffImages && (!entity._versionBase_isVersion || (!entity._versionBase_hasFluff && !entity._versionBase_hasFluffImages))) return null;
		...
		if (entity._versionBase_isVersion && (entity._versionBase_hasFluff || entity._versionBase_hasFluffImages)) {
			return DataLoader.pCacheAndGet(fluffProp, SourceUtil.getEntitySource(entity), entity._versionBase_hash, {
				lockToken2,
			});
		}
	 ...
	 */
	static _getFluffTabContent_isSkipEntryName({ entity, fluff, ix, fluffEntry }) {
	...
	static prerequisite = class {
		static _WEIGHTS = ["level", "pact", "patron", "spell", "race", "alignment", "ability", "proficiency", "expertise", "spellcasting", "spellcasting2020", "spellcastingFeature", "spellcastingPrepared", "spellcastingFocus", "psionics", "feature", "feat", "background", "item", "itemType", "itemProperty", "campaign", "culture", "group", "other", "otherSummary", undefined].mergeMap((k, i) => ({ [k]: i }));
		 ...
		 * @return {string}
		 */
		static getHtml(prerequisites, { isListMode = false, blocklistKeys = null, keyOptions = null, isTextOnly = false, isSkipPrefix = false, styleHint = null } = {}) {
			...
			const prereqsShared =
				prerequisites.length === 1
					? {}
					: Object.entries(prerequisites.slice(1).reduce((a, b) => CollectionUtil.objectIntersect(a, b), prerequisites[0]))
							.filter(([k, v]) => prerequisites.every((pre) => CollectionUtil.deepEquals(pre[k], v)))
							.mergeMap(([k, v]) => ({ [k]: v }));

			const shared = Object.keys(prereqsShared).length ? this.getHtml([prereqsShared], { isListMode, blocklistKeys, isTextOnly, isSkipPrefix: true }) : null;
				...
				.map((pr) => {
					...
					const prereqsToJoin = Object.entries(pr)
						...
						.map(([k, v]) => {
							...
							switch (k) {
								case "level":
									return this._getHtml_level({ v, isListMode, keyOptions, isTextOnly, styleHint });
								case "pact":
									return this._getHtml_pact({ v, isListMode, keyOptions, isTextOnly, styleHint });
								case "patron":
									return this._getHtml_patron({ v, isListMode, keyOptions, isTextOnly, styleHint });
								case "spell":
									return this._getHtml_spell({ v, isListMode, keyOptions, isTextOnly, styleHint });
								case "feat":
									return this._getHtml_feat({ v, isListMode, keyOptions, isTextOnly, styleHint });
								case "optionalfeature":
									return this._getHtml_optionalfeature({ v, isListMode, keyOptions, isTextOnly, styleHint });
								case "feature":
									return this._getHtml_feature({ v, isListMode, keyOptions, isTextOnly, styleHint });
								case "item":
									return this._getHtml_item({ v, isListMode, keyOptions, isTextOnly, styleHint });
								case "itemType":
									return this._getHtml_itemType({ v, isListMode, keyOptions, isTextOnly, styleHint });
								case "itemProperty":
									return this._getHtml_itemProperty({ v, isListMode, keyOptions, isTextOnly, styleHint });
								case "otherSummary":
									return this._getHtml_otherSummary({ v, isListMode, keyOptions, isTextOnly, styleHint });
								case "other":
									return this._getHtml_other({ v, isListMode, keyOptions, isTextOnly, styleHint });
								case "race":
									return this._getHtml_race({ v, isListMode, keyOptions, isTextOnly, styleHint });
								case "background":
									return this._getHtml_background({ v, isListMode, keyOptions, isTextOnly, styleHint });
								case "ability":
									return this._getHtml_ability({ v, isListMode, keyOptions, isTextOnly, styleHint });
								case "proficiency":
									return this._getHtml_proficiency({ v, isListMode, keyOptions, isTextOnly, styleHint });
								case "expertise":
									return this._getHtml_expertise({ v, isListMode, keyOptions, isTextOnly, styleHint });
								case "spellcasting":
									return this._getHtml_spellcasting({ v, isListMode, keyOptions, isTextOnly, styleHint });
								case "spellcasting2020":
									return this._getHtml_spellcasting2020({ v, isListMode, keyOptions, isTextOnly, styleHint });
								case "spellcastingFeature":
									return this._getHtml_spellcastingFeature({ v, isListMode, keyOptions, isTextOnly, styleHint });
								case "spellcastingPrepared":
									return this._getHtml_spellcastingPrepared({ v, isListMode, keyOptions, isTextOnly, styleHint });
								case "spellcastingFocus":
									return this._getHtml_spellcastingFocus({ v, isListMode, keyOptions, isTextOnly, styleHint });
								case "psionics":
									return this._getHtml_psionics({ v, isListMode, keyOptions, isTextOnly, styleHint });
								case "alignment":
									return this._getHtml_alignment({ v, isListMode, keyOptions, isTextOnly, styleHint });
								case "campaign":
									return this._getHtml_campaign({ v, isListMode, keyOptions, isTextOnly, styleHint });
								case "culture":
									return this._getHtml_culture({ v, isListMode, keyOptions, isTextOnly, styleHint });
								case "group":
									return this._getHtml_group({ v, isListMode, keyOptions, isTextOnly, styleHint });
								default:
			...
			const ptsSharedOtherJoiner = ptsSharedOther.some((pt) => pt.includes(",")) ? (ptsSharedOther.some((pt) => pt.includes(";")) ? "; plus " : "; ") : ", ";
		...
		static _getHtml_level({ v, isListMode, keyOptions, isTextOnly, styleHint }) {
			...
			const isLevelVisible =
				v.level !== 1 && // Hide the "implicit" 1st level.
			...
			const ptLevel = !isLevelVisible ? "" : styleHint === "classic" ? `${Parser.getOrdinalForm(v.level)} level` : `Level ${v.level}+`;
				...
				.map((uid) => {
					uid =
						styleHint === "classic"
							...
							: uid
									.split("|")
									.map((pt, i) => (i === 0 ? pt.toTitleCase() : pt))
									.join("|");
					const asTag = `{@${tag} ${uid}}`;
			...
			const parts = v.map((obj) => {
				return Object.entries(obj).map(([profType, prof]) => {
					switch (profType) {
						...
						default:
							throw new Error(`Unhandled expertise type: "${profType}"`);
					}
				});
			});
			return isListMode ? parts.join("/") : parts.joinConjunct(", ", " or ");
		...
		static _getHtml_culture({ v, isListMode }) {
			return isListMode ? v.join("/") : `${v.joinConjunct(", ", " or ")} Culture`;
		}
	...
	static getSensesEntry(senses, { isTitleCase = false } = {}) {
		...
		if (!Renderer.utils._FN_TAG_SENSES) {
			Renderer.utils._SENSE_TAG_METAS = [...MiscUtil.copyFast(Parser.getSenses()), ...(PrereleaseUtil.getBrewProcessedFromCache("sense") || []), ...(BrewUtil2.getBrewProcessedFromCache("sense") || [])];
			...
			Renderer.utils._FN_TAG_SENSES = (str) => {
				Renderer.utils._SENSE_TAG_METAS.forEach(({ name, source, _re }) => (str = str.replace(_re, (...m) => `{@sense ${m.last().sense[isTitleCase ? "toTitleCase" : "toString"]()}|${source}}`)));
				return str;
			};
		}
	...
	static getTagEntry(tag, text) {
		switch (tag) {
			...
			case "@savingThrow": {
				// format: {@savingThrow str 5} or {@savingThrow str 5|Display Text} or {@savingThrow str 5|Display Text|Roll Name Text}
				const fauxEntry = {
					type: "dice",
					rollable: true,
					subType: "d20",
				...
				let [abil, ...rawScoreOrModParts] = abilAndScoreOrScore
					.split(" ")
					.map((it) => it.trim())
					.filter(Boolean);
					...
					if (type && value) {
						switch (type) {
							case VeCt.HASH_SCALED: {
								...
								out.subhashes = [{ key: VeCt.HASH_SCALED, value: targetCrNum }];
							...
							case VeCt.HASH_SCALED_SPELL_SUMMON: {
								...
								out.subhashes = [{ key: VeCt.HASH_SCALED_SPELL_SUMMON, value: scaledSpellNum }];
							...
							case VeCt.HASH_SCALED_CLASS_SUMMON: {
								...
								out.subhashes = [{ key: VeCt.HASH_SCALED_CLASS_SUMMON, value: scaledClassNum }];
	...
	static lazy = {
		...
		getCreateObserver({ observerId, fnOnObserve }) {
			if (!Renderer.utils.lazy._OBSERVERS[observerId]) {
				...
				observer.getPrintPromises = () => {
					...
					return [...observer._TRACKED].map((it) => {
						observer.untrack(it);
						return fnOnObserve({
							observer,
							entry: {
		...
		_initPrintListener() {
			...
			Renderer.utils.lazy._printListener = () => {
				const promises = Object.values(Renderer.utils.lazy._OBSERVERS).flatMap((observer) => observer.getPrintPromises());
		...
		mutFinalizeEle(ele) {
			...
			if (ele.tagName === "CANVAS") {
				...
				pLoad.then(() => ctx.drawImage(image, 0, 0, ele.width, ele.height));
...
Renderer.tag = class {
	...
	static _TagTextStyle = class extends this._TagBaseAt {
		_getStripped(tag, text) {
			return Renderer.splitTagByPipe(text)[0] || "";
		}
	};
	...
	static TagFont = class extends this._TagTextStyle {
		...
		static getFontInfo(fontDecl) {
			...
			const fontUrl = /^https?:\/\//.test(fontDecl) ? fontDecl : Renderer.get().getMediaUrl("fonts", fontDecl);
	...
	static TagActSave = class extends this._TagBaseAt {
		...
		_getStripped(tag, text) {
			return `${Parser.attAbvToFull(text)} Saving Throw:`;
		}
	};
	...
	static TagActResponse = class extends this._TagBaseAt {
		...
		_getStripped(tag, text) {
			return `Response${text.includes("d") ? "\u2014" : ":"}`;
		}
	};
	...
	static TagAtkr = class extends this._TagBaseAt {
		...
		_getStripped(tag, text) {
			return Renderer.attackTagToFull(text, { isRoll: true });
		}
	};
...
Renderer.events = class {
	...
	static _handleClick_headerToggleButton_doToggleEle(ele, { selector = false, force = null } = {}) {
		...
		while (eleNxt) {
			...
			if (selector !== `[data-rd-h-special-toggle-button]`) {
				...
				if (eleToCheck.classList.contains("rd__b-special") || (eleToCheck.classList.contains("rd__h") && !eleToCheck.classList.contains("rd__h--3"))) break;

				if (!eleToCheck.classList.contains("rd__b") || eleToCheck.classList.contains("rd__b--3")) {
				...
				if ([...eleToCheck.querySelectorAll(".rd__h")].some((eleSub) => eleSub.classList.contains("rd__h--0") || eleSub.classList.contains("rd__h--1") || eleSub.classList.contains("rd__h--2"))) break;
			}
	...
	static _handleLoad_inlineStatblock_fnOnObserve({ entry }) {
		...
		return DataLoader.pCacheAndGet(page, Parser.getTagSource(tag, source), hash).then((toRender) => {
			...
			const nxt = e_({
				outer:
					Renderer.utils.getEmbeddedDataHeader(headerName, style, {
...
class _RenderCompactImplBase {
	...
	_getCommonHtmlParts({ ent, renderer, opts }) {
		return {
...
class _RenderCompactFeatsImplBase extends _RenderCompactImplBase {
	...
	_getCommonHtmlParts_prerequisites({ ent }) {
		const ptCategoryPrerequisite = Renderer.feat.getJoinedCategoryPrerequisites(ent.category, Renderer.utils.prerequisite.getHtml(ent.prerequisite, { styleHint: this._style }));
...
class _RenderCompactFeatsImplClassic extends _RenderCompactFeatsImplBase {
	...
	_getCompactRenderedString({ ent, renderer, opts }) {
		...
		const { htmlPtRepeatable } = this._getHtmlParts({
			ent,
			renderer,
		});

		const ptHeader =
			htmlPtPrerequisites || htmlPtRepeatable
...
Renderer.feat = class {
	...
	static _mergeAbilityIncrease_getText(abilityObj) {
		...
		if (abilityObj.choose.weighted) {
			const ptsWeight = abilityObj.choose.weighted.weights.map((adj, i) => `${i === 0 ? "an" : "another"} ability score to ${adj > 0 ? "increase" : "decrease"} by ${Math.abs(adj)}`).joinConjunct(", ", " and ");
	...
	static getCompactRenderedString(ent, opts) {
		...
		switch (styleHint) {
			case "classic":
				return this._RENDER_CLASSIC.getCompactRenderedString(ent, opts);
			default:
...
Renderer.class = class {
	...
	static getHitDiceEntry(clsHd) {
		return clsHd ? `{@dice ${clsHd.number}d${clsHd.faces}||Hit die}` : null;
	}
	...
	static getHitPointsAtHigherLevels(className, clsHd) {
		return className && clsHd ? `${Renderer.get().render(Renderer.class.getHitDiceEntry(clsHd))} (or ${(clsHd.number * clsHd.faces) / 2 + 1}) + your Constitution modifier per ${className} level after 1st` : null;
	}
	...
	static _getHtmlPtStartingEquipment_default({ equip, renderer }) {
		return [equip.additionalFromBackground ? "<p>You start with the following items, plus anything provided by your background.</p>" : "", equip.default && equip.default.length ? `<ul class="pl-4"><li>${equip.default.map((it) => renderer.render(it)).join("</li><li>")}</ul>` : "", equip.goldAlternative != null ? `<p>Alternatively, you may start with ${renderer.render(equip.goldAlternative)} gp to buy your own equipment.</p>` : ""].filter(Boolean).join("");
	}
	...
	static _getHtmlPtStartingEquipment_entries({ equip, renderer, styleHint }) {
		...
		return renderer.render({
			type: "entries",
			entries: [`{@b Starting Equipment:} ${firstEntry}`, ...otherEntries],
		});
	}
	...
	static getDisplayNamedSubclassFeatureEntry(ent, styleHint) {
		...
		cpy.entries = cpy.entries.map((ent) => {
		...
		cpyCls.classFeatures = cpyCls.classFeatures.map((lvlFeatures, ixLvl) => {
			return walker.walk(lvlFeatures, {
				object: (obj) => {
					...
					const isDisplay = [obj.source, ...(obj.otherSources || []).map((it) => it.source)].some((src) =>
						pageFilter.filterBox.toDisplayByFilters(
							filterValues,
							...[
								{
...
Renderer.spell = class {
	...
	static getHtmlPtRange(spell, { styleHint = null, isDisplaySelfArea = false } = {}) {
		return `<b>Range:</b> ${Parser.spRangeToFull(spell.range, { styleHint, isDisplaySelfArea })}`;
	}
	static getHtmlPtComponents(spell) {
		return `<b>Components:</b> ${Parser.spComponentsToFull(spell.components, spell.level)}`;
	}
	static getHtmlPtDuration(spell, { styleHint = null } = {}) {
		return `<b>Duration:</b> ${Parser.spDurationToFull(spell.duration, { styleHint })}`;
	}
	...
	static _SpellSourceManager = class {
		...
		populate({ brew, isForce = false }) {
			...
			(brew.spellList || []).forEach((spellList) => this._populate_fromGroup_group({ spellList }));
		}

		_populate_fromClass_classSubclass({ itm, className, classSource, subclassShortName = null, subclassName = null, subclassSource = null, subSubclassName = null }) {
			...
			this._populate_fromClass_doAdd({
				tgt: MiscUtil.getOrSet(this._cache.classes, "spell", source, name, { fromClassList: [], fromSubclass: [] }),
				className,
				classSource,
				subclassShortName,
				subclassName,
		...
		_populate_fromClass_doAdd({ tgt, className, classSource, subclassShortName, subclassName, subclassSource, subSubclassName, schools = null }) {
		...
		_populate_fromClass_group({ itm, className, classSource, subclassShortName = null, subclassName = null, subclassSource = null, subSubclassName = null }) {
			if (!itm.groupName) return;
		...
		_mutateSpell_brewGeneric({ sp, lowName, lowSource, propSpell, prop }) {
			...
			if (this._cache[propSpell]?.spell?.[lowSource]?.[lowName]?.length) {
				(sp[propTmp] ||= []).push(...this._cache[propSpell].spell[lowSource][lowName]);
			}
	...
	static isExcludedSubclassVariantSource({ classDefinedInSource, subclassDefinedInSource }) {
		return (classDefinedInSource != null && ExcludeUtil.isExcluded("*", "classFeature", classDefinedInSource, { isNoCount: true })) || (subclassDefinedInSource != null && ExcludeUtil.isExcluded("*", "subclassFeature", subclassDefinedInSource, { isNoCount: true }));
	}

	static getCombinedGeneric(sp, { propSpell, prop = null }) {
		const propSpellTmp = `_tmp${propSpell.uppercaseFirst()}`;
		return [...(sp[propSpell] || []), ...(sp[propSpellTmp] || [])]
			.filter((it) => {
...
Renderer.condition = class {
	static getCompactRenderedString(ent) {
		return Renderer.conditionDisease.getCompactRenderedString(ent);
	}
	static pGetFluff(ent) {
		return Renderer.conditionDisease.pGetFluff(ent);
	}
};
...
class _RenderCompactOptionalfeaturesImplBase extends _RenderCompactImplBase {
	...
	_getCompactRenderedString({ ent, renderer, opts }) {
		...
		const ptHeader =
			htmlPtPrerequisites || htmlPtCost
				? `<tr><td colspan="6" class="pb-2 pt-0">
			${htmlPtPrerequisites}
			${htmlPtCost}
		</td></tr>`
...
Renderer.optionalfeature = class {
	...
	static getTypeText(ent) {
		const commonPrefix =
			ent.featureType.length > 1
				? MiscUtil.findCommonPrefix(
						ent.featureType.map((fs) => Parser.optFeatureTypeToFull(fs)),
...
Renderer.race = class {
	static getRaceRenderableEntriesMeta(ent, { styleHint = null } = {}) {
		...
		const entsAttributes = [ent.abilityEntry || (ent.ability ? { type: "item", name: "Ability Scores:", entry: Renderer.getAbilityData(ent.ability).asText } : null), ent.creatureTypesEntry || this._getRaceRenderableEntriesMeta_creatureType({ ent, styleHint }), ent.sizeEntry || (ent.size ? { type: "item", name: "Size:", entry: Renderer.utils.getRenderedSize(ent.size || [Parser.SZ_VARIES]) } : null), ent.speedEntry || (ent.speed != null ? { type: "item", name: "Speed:", entry: Parser.getSpeedString(ent, { isLongForm: true }) } : null)].filter(Boolean);

		return {
			entryAttributes: entsAttributes.length ? { type: "list", style: "list-hang-notitle", items: entsAttributes } : null,
			entryMain: ent._isBaseRace ? { type: "entries", entries: ent._baseRaceEntries } : { type: "entries", entries: ent.entries },
		};
	}
	...
	static bindListenersHeightAndWeight(race, ele) {
		...
		const pRollWeight = async () => {
			...
			const mResultWeightMod = isNaN(weightModRaw)
				? await Renderer.dice.pRoll2(weightModRaw, {
						isUser: false,
		...
		return {
			entriesAttributes: [
				...Object.entries(Renderer.deity._BASE_PART_TRANSLATORS)
					...
					.filter(Boolean),
				...Object.entries(ent.customProperties || {}).map(([name, val]) => ({
					name,
					entry: `{@b ${name}:} ${val}`,
				})),
			]
...
Renderer.object = class {
	...
	static RENDERABLE_ENTRIES_PROP_ORDER__ATTRIBUTES = ["entryCreatureCapacity", "entryCargoCapacity", "entryArmorClass", "entryHitPoints", "entrySpeed", "entryAbilityScores", "entryDamageImmunities", "entryDamageResistances", "entryDamageVulnerabilities", "entryConditionImmunities", "entrySenses"];
	...
	static getObjectRenderableEntriesMeta(ent) {
		return {
			...
			entryAbilityScores: Parser.ABIL_ABVS.some((ab) => ent[ab] != null)
				? `{@b Ability Scores:} ${Parser.ABIL_ABVS.filter((ab) => ent[ab] != null)
						.map((ab) => `${ab.toUpperCase()}\u00A0${Renderer.utils.getAbilityRollerEntry(ent, ab)}`)
						.join(", ")}`
				: null,
			...
			entryConditionImmunities: ent.conditionImmune ? `{@b Condition Immunities:} ${Parser.getFullCondImm(ent.conditionImmune, { isEntry: true })}` : null,
			entrySenses: ent.senses ? `{@b Senses:} ${Renderer.utils.getSensesEntry(ent.senses)}` : null,
		};
	}
	...
	static getRenderedString(ent, opts) {
		...
		const ptAttribs = Renderer.object.RENDERABLE_ENTRIES_PROP_ORDER__ATTRIBUTES.filter((prop) => entriesMeta[prop])
			.map((prop, i) => {
...
Renderer.trap = class {
			...
			const listItems = [
				...
				ent.duration
					? {
							type: "item",
							name: "Duration:",
							entries: [Renderer.generic.getRenderableDurationEntriesMeta(ent.duration, { styleHint }).entryDuration],
						}
					: null,
				ent.hauntBonus
					? {
	...
	static getRenderedTrapBody(renderer, ent, { styleHint = null } = {}) {
		...
		return (isPrefixTrapPart ? [ptTrap, ptEntries] : [ptEntries, ptTrap]).filter(Boolean).join("");
	}
...
Renderer.traphazard = class {
	static getSubtitle(ent, { styleHint } = {}) {
		...
		return ent.rating
			.map((rating) => {
				...
				const ptThreatType = [ptThreat, ptType].filter(Boolean).join(" ");

				const ptLevelTier = Renderer.traphazard.getRenderedTrapHazardRatingPart(rating, { styleHint });

				return [ptThreatType, ptLevelTier ? `(${ptLevelTier})` : ""].filter(Boolean).join(" ");
			})
			.filter(Boolean)
			.joinConjunct(", ", " or ");
	}
	...
	static getCompactRenderedString(ent, opts) {
		...
		const ptBody = ent.__prop === "trap" ? Renderer.trap.getRenderedTrapBody(renderer, ent, { styleHint }) : Renderer.hazard.getRenderedHazardBody(renderer, ent, { styleHint });
		return `
...
Renderer.cultboon = class {
	...
	static getCompactRenderedString(ent) {
		...
		switch (ent.__prop) {
			case "cult":
				return Renderer.cultboon._getCompactRenderedString_cult({ ent, renderer });
			case "boon":
				return Renderer.cultboon._getCompactRenderedString_boon({ ent, renderer });
			default:
...
class _RenderCompactBestiaryImplBase {
	...
	_getCommonHtmlParts_name({ mon, opts, isInlinedToken }) {
		return Renderer.utils.getNameTr(mon, {
...
class _RenderCompactBestiaryImplClassic extends _RenderCompactBestiaryImplBase {
	...
	_getCompactRenderedString({ mon, renderer, opts }) {
		...
		const { entsTrait, entsAction, entsBonusAction, entsReaction, entsLegendaryAction, entsMythicAction, legGroup } = Renderer.monster.getSubEntries(mon, { renderer });
...
Renderer.monster = class {
	...
	static getPronounSubject(mon) {
		return mon.isNamedCreature ? "they" : "it";
	}
	static getPronounObject(mon) {
		return mon.isNamedCreature ? "them" : "its";
	}
	static getPronounPossessive(mon) {
	...
	static getLegendaryActionIntroEntry(mon, { isUseDisplayName = false, styleHint = null } = {}) {
		...
		return {
			entries: [`${legendaryNameTitle} can take ${legendaryActions} legendary action${legendaryActions > 1 ? "s" : ""}${legendaryActionsLair !== legendaryActions ? ` (or ${legendaryActionsLair} when in ${proPossessive} lair)` : ""}, choosing from the options below. Only one legendary action can be used at a time and only at the end of another creature's turn. ${legendaryNameTitle} regains spent legendary actions at the start of ${proPossessive} turn.`],
		};
	}
	...
	static getCrScaleTarget({ win, btnScale, $btnScale, initialCr, cbRender, isCompact }) {
	...
	static getSelSummonSpellLevel(mon) {
		...
		return e_({
			...
			children: [
				...
				...[...new Array(VeCt.SPELL_LEVEL_MAX + 1 - mon.summonedBySpellLevel)].map((_, i) =>
					e_({
						tag: "option",
	...
	static getSelSummonClassLevel(mon) {
		...
		return e_({
			...
			children: [
				e_({ tag: "option", val: "-1", text: "\u2014" }),
				...[...new Array(VeCt.LEVEL_MAX)].map((_, i) =>
					e_({
						tag: "option",
						val: i + 1,
						text: i + 1,
	...
	static getCompactRenderedStringSection({ ent, renderer, title, key, depth, styleHint, isHangingList = false }) {
		if (!ent[key]) return "";
		...
		const entriesArr = key === "lairActions" || key === "regionalEffects" ? [{ type: "entries", entries: ent[key] }] : ent[key];
	...
	static _getCompactRenderedStringSection_getRenderedContent({ renderer, depth, isHangingList, entriesArr }) {
		if (isHangingList) {
			const toRender = {
				type: "list",
				style: "list-hang-notitle",
				items: MiscUtil.copy(entriesArr).map((entSub) => {
	...
	static getInitiativeBonusNumber({ mon }) {
		...
		const profBonus = mon.initiative.proficiency && Parser.crToNumber(mon.cr) < VeCt.CR_CUSTOM ? mon.initiative.proficiency * Parser.crToPb(mon.cr) : 0;
	...
	static getSensesPart(mon, { isTitleCase = false, isForcePassive = false } = {}) {
		...
		const pts = [mon.senses ? Renderer.utils.getRenderedSenses(mon.senses, { isTitleCase }) : "", passive != null ? `${isTitleCase ? "Passive" : "passive"} Perception ${passive}` : isForcePassive || mon.senses ? "\u2014" : ""].filter(Boolean);
		return pts.join(", ");
	}
	...
	static getChallengeRatingPart(mon, { styleHint = null, isPlainText = false } = {}) {
		...
		switch (styleHint) {
			case "classic":
				return this._getChallengeRatingPart_classic({ mon, isPlainText });
			default:
				throw new Error(`Unhandled style "${styleHint}"!`);
		}
	}
	...
	static _getChallengeRatingPart_classic_getBasicCrRender({ cr = null, xp = null, isMythic = false } = {}) {
		...
		xp ??= Parser.crToNumber(cr) < VeCt.CR_CUSTOM ? Parser.crToXpNumber(cr) : null;
		const xpMythic = xp != null && isMythic ? (Parser.crToXpNumber(cr) != null ? Parser.crToXpNumber(cr) * 2 : null) : null;

		const ptXp = xp != null ? xp.toLocaleString() : null;
		const ptXpMythic = xpMythic != null ? xpMythic.toLocaleString() : null;

		const ptXps = [ptXp != null ? `${ptXp} XP` : null, ptXpMythic != null ? `${ptXpMythic} XP as a mythic encounter` : null].filter(Boolean).joinConjunct(", ", ", or ", true);
	...
	static _getChallengeRatingPart_classic({ mon, isPlainText = false } = {}) {
		...
		return stack.filter(Boolean).joinConjunct(", ", " or ");
	}
	...
	static _getRenderedAbilityScores_getSpecialMeta({ mon }) {
		...
		const ptsSpecial = Parser.ABIL_ABVS.map((abv) => {
	...
	static getRenderedAbilityScores(mon, { styleHint = null, renderer = null } = {}) {
		...
		switch (styleHint) {
			case "classic":
				return Renderer.monster._getRenderedAbilityScores_classic({ mon, renderer });
			default:
				throw new Error(`Unhandled style "${styleHint}"!`);
		}
	}
	...
	static getOrderedReactions(mon, { fnGetSpellTraits } = {}) {
		return Renderer.monster._getOrderedActionsBonusActions({ mon, fnGetSpellTraits, prop: "reaction" });
	}
	static getOrderedLegendaryActions(mon, { fnGetSpellTraits } = {}) {
		return Renderer.monster._getOrderedActionsBonusActions({ mon, fnGetSpellTraits, prop: "legendary" });
	}
	static getOrderedMythicActions(mon, { fnGetSpellTraits } = {}) {
		return Renderer.monster._getOrderedActionsBonusActions({ mon, fnGetSpellTraits, prop: "mythic" });
	}
	...
	static postProcessFluff(mon, fluff) {
		...
		const isAddName = thisGroup.mythicEncounter || [thisGroup.lairActions, thisGroup.regionalEffects].filter(Boolean).length !== 1;
	...
	static hover = class {
		static bindFluffImageMouseover({ mon, $ele }) {
			$ele.on("mouseover", (evt) => this._pOnFluffImageMouseover({ evt, mon, $ele }));
		}
...
Renderer.legendaryGroup = class {
	...
	static getSummaryEntry(legGroup) {
		...
		return {
			type: "section",
			entries: [legGroup.lairActions ? { name: "Lair Actions", type: "entries", entries: legGroup.lairActions } : null, legGroup.regionalEffects ? { name: "Regional Effects", type: "entries", entries: legGroup.regionalEffects } : null, legGroup.mythicEncounter ? { name: "As a Mythic Encounter", type: "entries", entries: legGroup.mythicEncounter } : null].filter(Boolean),
		};
	}
};
...
Renderer.item = class {
	...
	static _getPropertiesText_unusedDmg2({ item, renderer }) {
		return `alt. ${Renderer.item._renderDamage(item.dmg2, { renderer })}`;
	}
	static _getPropertiesText_unusedRange({ item, renderer }) {
	...
	static getRenderedDamageAndProperties(item, { renderer = null } = {}) {
		...
		// armor
		if (item.ac != null) {
			const dexterityMax = itemTypeAbv === Parser.ITM_TYP_ABV__MEDIUM_ARMOR && item.dexterityMax === undefined ? 2 : item.dexterityMax;
		...
		// damage
		if (item.dmg1) {
			damageParts.push([Renderer.item._renderDamage(item.dmg1, { renderer }), item.dmgType ? Parser.dmgTypeToFull(item.dmgType) : ""].filter(Boolean).join(" "));
		}
		...
		if (item.barDimensions) {
			damageParts.push([item.barDimensions.l ? `${Parser.getInchesToFull(item.barDimensions.l, { isShort: true })} long` : "", item.barDimensions.w ? `${Parser.getInchesToFull(item.barDimensions.w, { isShort: true })} wide` : "", item.barDimensions.h ? `${Parser.getInchesToFull(item.barDimensions.h, { isShort: true })} thick` : ""].filter(Boolean).join(" × "));
		}
	...
	static getHtmlAndTextTypes(item, { styleHint = null } = {}) {
		...
		if (item.staff && itemTypeAbv !== Parser.ITM_TYP_ABV__MELEE_WEAPON && itemTypeAltAbv !== Parser.ITM_TYP_ABV__MELEE_WEAPON) {
	...
	static getRenderedEntries(item, { isCompact = false, wrappedTypeAllowlist = null } = {}) {
		...
		Renderer.item._GET_RENDERED_ENTRIES_WALKER =
			Renderer.item._GET_RENDERED_ENTRIES_WALKER ||
			MiscUtil.getWalker({
		...
		const handlersVariantName =
			item._variantName == null
				? null
				: {
		...
		return `<div class="ve-flex-col">
			${
				typeRarityText || tierText
					? `<div class="split ${subTypeText ? "mb-1" : ""}">
	...
	static getProperty(uid, { isIgnoreMissing = false } = {}) {
		...
		const out =
			this._propertyMap?.[source]?.[abbreviation] ||
			// TODO(Future; 2025-Q2) remove once all prerelease/homebrew migrated
	...
	static _addType(ent) {
		...
		if (existing) {
			Object.entries(ent).forEach(([k, v]) => (cpy[k] ??= MiscUtil.copyFast(v)));
		}
	...
	static getType(uid, { isIgnoreMissing = false } = {}) {
		...
		const out =
			this._typeMap?.[source]?.[abbreviation] ||
			// TODO(Future; 2025-Q2) remove once all prerelease/homebrew migrated
			// Fall back on sourceless tag
	...
	static addPrereleaseBrewPropertiesAndTypesFrom({ data }) {
		...
		(data.itemTypeAdditionalEntries || []).forEach((it) => Renderer.item._addAdditionalTypeEntries(it));
	...
	static async _pGetCacheSiteGenericVariants() {
		Renderer.item._pGettingSiteGenericVariants =
			Renderer.item._pGettingSiteGenericVariants ||
			(async () => {
		...
		Object.entries(inherits)
			...
			.forEach(([inheritedProperty, val]) => {
				switch (inheritedProperty) {
					...
					case "propertyAdd": {
						specificVariant.property = [...(specificVariant.property || []), ...val.filter((it) => !specificVariant.property || !specificVariant.property.some((property) => (property?.uid || property) === (it?.uid || it)))];
						break;
					}
					case "propertyRemove": {
	...
	static enhanceItem(item, { styleHint = null } = {}) {
		...
		if (item.type === Parser.ITM_TYP__TOOL || item.type === Parser.ITM_TYP__ARTISAN_TOOL || item.type === Parser.ITM_TYP__INSTRUMENT || item.type === Parser.ITM_TYP__GAMING_SET) {
	...
	static async pGetSiteUnresolvedRefItemsFromPrereleaseBrew({ brewUtil, brew = null }) {
		...
		if (brew.baseitem && brew.baseitem.length) {
			...
			const variants = await Renderer.item.pGetGenericAndSpecificVariants(genericVariants, { baseItems: brew.baseitem || [], isSpecificVariantsOnly: true });
		...
		if (brew.magicvariant && brew.magicvariant.length) {
			...
			const variants = await Renderer.item.pGetGenericAndSpecificVariants(brew.magicvariant, { additionalBaseItems: brew.baseitem || [] });
			items = [...items, ...variants];
		}
	...
	static getFlatAttachedSpells(item) {
		...
		const out = [];
		Object.entries(item.attachedSpells).forEach(([useType, v]) => {
			switch (useType) {
				case "will":
				case "ritual":
				case "other": {
				...
				default:
					throw new Error(`Unhandled "attachedSpells" key "${useType}"!`);
			}
		});
		return out;
	}
...
Renderer.table = class {
	...
	static getHeaderRowMetas(ent) {
		...
		const lenPer = ent.colLabelRows.map((row) => Renderer.table.getHeaderRowSpanWidth(row));
...
Renderer.vehicle = class {
	...
	static getRenderedString(ent, opts) {
		...
		switch (ent.vehicleType) {
			...
			case "CREATURE":
				return Renderer.monster.getCompactRenderedString(ent, { ...opts, isHideLanguages: true, isHideSenses: true, isCompact: opts.isCompact ?? false, page: UrlUtil.PG_VEHICLES });
			case "OBJECT":
				return Renderer.object.getCompactRenderedString(ent, { ...opts, isCompact: opts.isCompact ?? false, page: UrlUtil.PG_VEHICLES });
			default:
	...
	static spelljammer = class {
		static getVehicleSpelljammerRenderableEntriesMeta(ent) {
			...
			const ptSpeed = ent.speed != null ? Parser.getSpeedString(ent, { isSkipZeroWalk: true }) : "";
			...
			return {
				entryTableSummary: {
					...
					rows: [
						[`{@b Armor Class:} ${ptAc}`, `{@b Cargo:} ${ent.capCargo ? `${ent.capCargo} ton${ent.capCargo === 1 ? "" : "s"}` : "\u2014"}`],
						[`{@b Hit Points:} ${ent.hull?.hp ?? "\u2014"}`, `{@b Crew:} ${ent.capCrew ?? "\u2014"}${ent.capCrewNote ? ` ${ent.capCrewNote}` : ""}`],
						[`{@b Damage Threshold:} ${ent.hull?.dt ?? "\u2014"}`, `{@b Keel/Beam:} ${(ent.dimensions || ["\u2014"]).join("/")}`],
						[`{@b Speed:} ${ptSpeedPace}`, `{@b Cost:} ${ent.cost != null ? Parser.vehicleCostToFull(ent) : "\u2014"}`],
					],
				},
			};
		}
	...
	static _getAbilitySection(veh) {
		return Parser.ABIL_ABVS.some((it) => veh[it] != null)
			? `<tr><td colspan="6">
	...
	static _getResImmVulnSection(ent, { entriesMeta = null } = {}) {
		...
		const props = ["entryDamageVulnerabilities", "entryDamageResistances", "entryDamageImmunities", "entryConditionImmunities"];
		...
		return `<tr><td colspan="6" class="pb-2">
			${props
				...
				.map((prop) => `<div>${Renderer.get().render(entriesMeta[prop])}</div>`)
				.join("")}
		</td></tr>`;
	}
	...
	static _getTraitSection(renderer, veh) {
		return veh.trait
			? `<tr><td colspan="6"><h3 class="stats__sect-header-inner">Traits</h3></td></tr>
		<tr><td colspan="6" class="pt-2 pb-2">
		${Renderer.monster
	...
	static _getRenderedString_ship(ent, opts) {
		...
		return `
			...
			${
				ent.hull
					? `${Renderer.vehicle.ship.getSectionTitle_("Hull")}
			<tr><td colspan="6" class="stats__sect-row-inner">
	...
	static infwar = class {
		...
		static getVehicleInfwarRenderableEntriesMeta(ent) {
			...
			const ptDtMt = [ent.hp.dt != null ? `damage threshold ${ent.hp.dt}` : null, ent.hp.mt != null ? `mishap threshold ${ent.hp.mt}` : null].filter(Boolean).join(", ");
...
Renderer.vehicleUpgrade = class {
	static getUpgradeSummary(ent, { styleHint = null } = {}) {
		...
		return [ent.upgradeType ? ent.upgradeType.map((t) => Parser.vehicleTypeToFull(t)) : null, ent.prerequisite ? Renderer.utils.prerequisite.getHtml(ent.prerequisite, { styleHint }) : null].filter(Boolean).join(", ");
	}
...
Renderer.recipe = class {
	static _getEntryMetasTime(ent) {
		...
		return ["total", "preparation", "cooking", ...Object.keys(ent.time)]
			.unique()
	...
	static getRecipeRenderableEntriesMeta(ent) {
		return {
			...
			entryServes: ent.serves ? `{@b {@style Serves|small-caps}} ${ent.serves.min ?? ent.serves.exact}${ent.serves.min != null ? " to " : ""}${ent.serves.max ?? ""}${ent.serves.note ? ` ${ent.serves.note}` : ""}` : null,
			entryMetasTime: Renderer.recipe._getEntryMetasTime(ent),
			entryIngredients: { entries: ent._fullIngredients },
			entryEquipment: ent._fullEquipment?.length ? { entries: ent._fullEquipment } : null,
			entryCooksNotes: ent.noteCook ? { entries: ent.noteCook } : null,
	...
	static getScaledRecipe(r, scaleFactor) {
		...
		["ingredients", "equipment"].forEach((prop) => {
			...
			MiscUtil.getWalker({ keyBlocklist: MiscUtil.GENERIC_WALKER_ENTRIES_KEY_BLOCKLIST }).walk(cpyR[prop], {
				object: (obj) => {
					...
					const amountsOriginal = Object.keys(objOriginal)
						.filter((k) => /^amount\d+$/.test(k))
						.map((k) => objOriginal[k]);
					const amountsScaled = Object.keys(obj)
						.filter((k) => /^amount\d+$/.test(k))
						.map((k) => obj[k]);
	...
	static _UNITS_SINGLE_TO_PLURAL_S = ["bundle", "cup", "handful", "ounce", "packet", "piece", "pound", "slice", "sprig", "square", "strip", "tablespoon", "teaspoon", "wedge", "fist"];
	...
	static _mutWrapOriginalAmounts({ obj, objOriginal }) {
		...
		obj.entry = parts
			.map((pt) =>
				pt.replace(Renderer.recipe._RE_AMOUNT, (...m) => {
...
Renderer.card = class {
	static getFullEntries(ent, { backCredit = null } = {}) {
		...
		const ptCredits = [ent.face?.credit ? `art credit: ${ent.face?.credit}` : null, backCredit || ent.back?.credit ? `art credit (reverse): ${backCredit || ent.back?.credit}` : null].filter(Boolean).join(", ").uppercaseFirst();
	...
	static getCompactRenderedString(ent) {
		...
		return `
			...
			${Renderer.get()
				...
				.render({ ...ent.face, maxHeight: 40, maxHeightUnits: "vh" })}
			${
				fullEntries?.length
					? `<hr class="hr-3">
...
Renderer.itemProperty = class {
	static getCompactRenderedString(ent) {
		...
		if (cpy.entries) {
			...
			const entries = cpy.entriesTemplate.length === 1 && cpy.entriesTemplate[0].name === faux.name && cpy.entriesTemplate[0].entries ? cpy.entriesTemplate[0].entries : cpy.entriesTemplate;
			...
			faux.entries = JSON.parse(JSON.stringify(entries).replace(/{{[^}]+}}/g, `{@i (${getNextLetter()})}`));
		} else faux.entries = [];
...
Renderer.generic = class {
	...
	static FEATURE__TOOLS_GAMING_SETS = ["dragonchess set", "dice set", "playing card set", "three-dragon ante set"];
	static _FEATURE__TOOL_GROUPS = new Set(["artisan's tools", "gaming set", "musical instrument"]);
	static FEATURE__TOOLS_ALL = ["artisan's tools", ...this.FEATURE__TOOLS_ARTISANS, "disguise kit", "forgery kit", "gaming set", ...this.FEATURE__TOOLS_GAMING_SETS, "herbalism kit", "musical instrument", ...this.FEATURE__TOOLS_MUSICAL_INSTRUMENTS, "navigator's tools", "thieves' tools", "poisoner's kit", "vehicles (land)", "vehicles (water)", "vehicles (air)", "vehicles (space)"];

	static FEATURE__LANGUAGES_ALL = Parser.LANGUAGES_ALL.map((it) => it.toLowerCase());
	static FEATURE__LANGUAGES_STANDARD__CHOICE_OBJ = {
		from: [
			...Parser.LANGUAGES_STANDARD.map((it) => ({
				name: it.toLowerCase(),
	...
	static _summariseProfs({ profGroupArr, skillToolLanguageProfs, setValid, setValidAny, anyAlt, isShort, hoverTag }) {
		...
		const summary = [
			...
			...(skillToolLanguageProfs || []).map((profGroup) => handleProfGroup(profGroup)),
		]
			.filter(Boolean)
			.join(` <i>or</i> `);
		...
		switch (keyAny) {
			case "anySkill":
				return {
					...
					from: this.FEATURE__SKILLS_ALL.map((it) => ({ name: it, prop: "skillProficiencies" })),
					count: mappedCount,
				};
			case "anyTool":
				return {
			...
			case "anyArtisansTool":
				return {
					...
					from: this.FEATURE__TOOLS_ARTISANS.map((it) => ({ name: it, prop: "toolProficiencies" })),
					count: mappedCount,
				};
			case "anyMusicalInstrument":
				return {
					name: mappedCount === 1 ? `Any Musical Instrument` : `Any ${mappedCount} Musical Instruments`,
					from: this.FEATURE__TOOLS_MUSICAL_INSTRUMENTS.map((it) => ({ name: it, prop: "toolProficiencies" })),
					count: mappedCount,
				};
			case "anyGamingSet":
				return {
					name: mappedCount === 1 ? `Any Gaming Set` : `Any ${mappedCount} Gaming Sets`,
					from: this.FEATURE__TOOLS_GAMING_SETS.map((it) => ({ name: it, prop: "toolProficiencies" })),
					count: mappedCount,
				};
			case "anyLanguage":
				return {
					name: mappedCount === 1 ? `Any Language` : `Any ${mappedCount} Languages`,
					from: this.FEATURE__LANGUAGES_ALL.map((it) => ({ name: it, prop: "languageProficiencies" })),
					count: mappedCount,
				};
			...
			case "anySavingThrow":
				return {
					...
					from: this.FEATURE__SAVING_THROWS_ALL.map((it) => ({ name: it, prop: "savingThrowProficiencies" })),
					count: mappedCount,
				};
	...
	static hasToken(ent, { isIgnoreImplicit = false } = {}) {
		const fromEntity =
			ent.tokenUrl || // TODO(Future) legacy; remove
...
Renderer.hover = class {
	...
	static _doInit() {
		if (!Renderer.hover._isInit) {
			...
			Renderer.hover._contextMenu = ContextUtil.getMenu([
				new ContextUtil.Action("Maximize All", () => {
				...
				null,
				new ContextUtil.Action("Close Others", () => {
	...
	static getHoverElementAttributes({ page, source, hash, preloadId = null, isFauxPage = false }) {
		return [`onmouseover="Renderer.hover.pHandleLinkMouseOver(event, this)"`, `onmouseleave="Renderer.hover.handleLinkMouseLeave(event, this)"`, `onmousemove="Renderer.hover.handleLinkMouseMove(event, this)"`, `onclick="Renderer.hover.handleLinkClick(event, this)"`, `ondragstart="Renderer.hover.handleLinkDragStart(event, this)"`, `data-vet-page="${page.qq()}"`, `data-vet-source="${source.qq()}"`, `data-vet-hash="${hash.qq()}"`, preloadId != null ? `data-vet-preload-id="${`${preloadId}`.qq()}"` : "", isFauxPage ? `data-vet-is-faux-page="true"` : "", Renderer.hover.getPreventTouchString()].filter(Boolean).join(" ");
	}
	...
	static async pHandleLinkMouseOver(evt, ele, opts) {
		...
		if (opts) {
			...
			({ page, source, hash, preloadId, isFauxPage } = Renderer.hover.getLinkElementData(ele));
		}
		...
		let toRender;
		if (preloadId != null) {
			// FIXME(Future) remove in favor of `customHashId`
			switch (page) {
		...
		meta.windowMeta = Renderer.hover.getShowWindow($content, Renderer.hover.getWindowPositionFromEvent(tmpEvt || evt, { isPreventFlicker: !meta.isPermanent }), {
			title: toRender?.name || "",
	...
	static handlePredefinedMouseOver(evt, ele, entryId, opts) {
		...
		meta.windowMeta = Renderer.hover.getShowWindow($content, Renderer.hover.getWindowPositionFromEvent(evt, { isPreventFlicker: !meta.isPermanent }), {
	...
	// (Baked into render strings)
	static handlePredefinedMouseLeave(evt, ele) {
		return Renderer.hover.handleLinkMouseLeave(evt, ele);
	}

	// (Baked into render strings)
	static handlePredefinedMouseMove(evt, ele) {
		return Renderer.hover.handleLinkMouseMove(evt, ele);
	}

	static _WINDOW_POSITION_PROPS_FROM_EVENT = ["isFromBottom", "isFromRight", "clientX", "window", "isPreventFlicker", "bcr"];
	...
	static getShowWindow($content, position, opts) {
		...
		const brdrTopRightResize = ee`<div class="hoverborder__resize-ne"></div>`.onn("mousedown", (evt) => Renderer.hover._getShowWindow_handleDragMousedown({ hoverWindow, hoverId, $hov, drag, $wrpContent }, { evt, type: 1, isResizeOnlyWidth })).onn("touchstart", (evt) => Renderer.hover._getShowWindow_handleDragMousedown({ hoverWindow, hoverId, $hov, drag, $wrpContent }, { evt, type: 1, isResizeOnlyWidth }));
		if (isResizeOnlyWidth) brdrTopRightResize.hideVe();

		const brdrRightResize = ee`<div class="hoverborder__resize-e"></div>`.onn("mousedown", (evt) => Renderer.hover._getShowWindow_handleDragMousedown({ hoverWindow, hoverId, $hov, drag, $wrpContent }, { evt, type: 2, isResizeOnlyWidth })).onn("touchstart", (evt) => Renderer.hover._getShowWindow_handleDragMousedown({ hoverWindow, hoverId, $hov, drag, $wrpContent }, { evt, type: 2, isResizeOnlyWidth }));

		const brdrBottomRightResize = ee`<div class="hoverborder__resize-se"></div>`.onn("mousedown", (evt) => Renderer.hover._getShowWindow_handleDragMousedown({ hoverWindow, hoverId, $hov, drag, $wrpContent }, { evt, type: 3, isResizeOnlyWidth })).onn("touchstart", (evt) => Renderer.hover._getShowWindow_handleDragMousedown({ hoverWindow, hoverId, $hov, drag, $wrpContent }, { evt, type: 3, isResizeOnlyWidth }));
		if (isResizeOnlyWidth) brdrBottomRightResize.hideVe();

		const brdrBtmResize = ee`<div class="hoverborder__resize-s"></div>`.onn("mousedown", (evt) => Renderer.hover._getShowWindow_handleDragMousedown({ hoverWindow, hoverId, $hov, drag, $wrpContent }, { evt, type: 4, isResizeOnlyWidth })).onn("touchstart", (evt) => Renderer.hover._getShowWindow_handleDragMousedown({ hoverWindow, hoverId, $hov, drag, $wrpContent }, { evt, type: 4, isResizeOnlyWidth }));
		...
		const brdrBtmLeftResize = ee`<div class="hoverborder__resize-sw"></div>`.onn("mousedown", (evt) => Renderer.hover._getShowWindow_handleDragMousedown({ hoverWindow, hoverId, $hov, drag, $wrpContent }, { evt, type: 5, isResizeOnlyWidth })).onn("touchstart", (evt) => Renderer.hover._getShowWindow_handleDragMousedown({ hoverWindow, hoverId, $hov, drag, $wrpContent }, { evt, type: 5, isResizeOnlyWidth }));
		if (isResizeOnlyWidth) brdrBtmLeftResize.hideVe();

		const brdrLeftResize = ee`<div class="hoverborder__resize-w"></div>`.onn("mousedown", (evt) => Renderer.hover._getShowWindow_handleDragMousedown({ hoverWindow, hoverId, $hov, drag, $wrpContent }, { evt, type: 6, isResizeOnlyWidth })).onn("touchstart", (evt) => Renderer.hover._getShowWindow_handleDragMousedown({ hoverWindow, hoverId, $hov, drag, $wrpContent }, { evt, type: 6, isResizeOnlyWidth }));

		const brdrTopLeftResize = ee`<div class="hoverborder__resize-nw"></div>`.onn("mousedown", (evt) => Renderer.hover._getShowWindow_handleDragMousedown({ hoverWindow, hoverId, $hov, drag, $wrpContent }, { evt, type: 7, isResizeOnlyWidth })).onn("touchstart", (evt) => Renderer.hover._getShowWindow_handleDragMousedown({ hoverWindow, hoverId, $hov, drag, $wrpContent }, { evt, type: 7, isResizeOnlyWidth }));
		if (isResizeOnlyWidth) brdrTopLeftResize.hideVe();

		const brdrTopResize = ee`<div class="hoverborder__resize-n"></div>`.onn("mousedown", (evt) => Renderer.hover._getShowWindow_handleDragMousedown({ hoverWindow, hoverId, $hov, drag, $wrpContent }, { evt, type: 8, isResizeOnlyWidth })).onn("touchstart", (evt) => Renderer.hover._getShowWindow_handleDragMousedown({ hoverWindow, hoverId, $hov, drag, $wrpContent }, { evt, type: 8, isResizeOnlyWidth }));
	...
	static _getShowWindow_setPosition({ $hov, $wrpContent, position }, positionNxt) {
		switch (positionNxt.mode) {
			...
			default:
				throw new Error(`Positioning mode unimplemented: "${positionNxt.mode}"`);
		}
	...
	static getEntityLink(ent, { displayText = null, prop = null, isLowerCase = false, isTitleCase = false } = {}) {
	...
	static getFnRenderCompact(page, { isStatic = false } = {}) {
		switch (page) {
			...
			case UrlUtil.PG_CLASSES:
				return Renderer.class.getCompactRenderedString.bind(Renderer.class);
			case UrlUtil.PG_SPELLS:
				return Renderer.spell.getCompactRenderedString.bind(Renderer.spell);
			case UrlUtil.PG_ITEMS:
				return Renderer.item.getCompactRenderedString.bind(Renderer.item);
			case UrlUtil.PG_BESTIARY:
				return (it) => Renderer.monster.getCompactRenderedString(it, { isShowScalers: !isStatic, isScaledCr: it._originalCr != null, isScaledSpellSummon: it._isScaledSpellSummon, isScaledClassSummon: it._isScaledClassSummon });
			case UrlUtil.PG_CONDITIONS_DISEASES:
				return Renderer.conditionDisease.getCompactRenderedString.bind(Renderer.conditionDisease);
			case UrlUtil.PG_BACKGROUNDS:
				return Renderer.background.getCompactRenderedString.bind(Renderer.background);
			case UrlUtil.PG_FEATS:
				return Renderer.feat.getCompactRenderedString.bind(Renderer.feat);
			case UrlUtil.PG_OPT_FEATURES:
				return Renderer.optionalfeature.getCompactRenderedString.bind(Renderer.optionalfeature);
			case UrlUtil.PG_PSIONICS:
				return Renderer.psionic.getCompactRenderedString.bind(Renderer.psionic);
			case UrlUtil.PG_REWARDS:
				return Renderer.reward.getCompactRenderedString.bind(Renderer.reward);
			case UrlUtil.PG_RACES:
				return (it) => Renderer.race.getCompactRenderedString(it, { isStatic });
			case UrlUtil.PG_DEITIES:
				return Renderer.deity.getCompactRenderedString.bind(Renderer.deity);
			case UrlUtil.PG_OBJECTS:
				return Renderer.object.getCompactRenderedString.bind(Renderer.object);
			case UrlUtil.PG_TRAPS_HAZARDS:
				return Renderer.traphazard.getCompactRenderedString.bind(Renderer.traphazard);
			case UrlUtil.PG_VARIANTRULES:
				return Renderer.variantrule.getCompactRenderedString.bind(Renderer.variantrule);
			case UrlUtil.PG_CULTS_BOONS:
				return Renderer.cultboon.getCompactRenderedString.bind(Renderer.cultboon);
			case UrlUtil.PG_TABLES:
				return Renderer.table.getCompactRenderedString.bind(Renderer.table);
			case UrlUtil.PG_VEHICLES:
				return Renderer.vehicle.getCompactRenderedString.bind(Renderer.vehicle);
			case UrlUtil.PG_ACTIONS:
				return Renderer.action.getCompactRenderedString.bind(Renderer.action);
			case UrlUtil.PG_LANGUAGES:
				return Renderer.language.getCompactRenderedString.bind(Renderer.language);
			case UrlUtil.PG_CHAR_CREATION_OPTIONS:
				return Renderer.charoption.getCompactRenderedString.bind(Renderer.charoption);
			case UrlUtil.PG_RECIPES:
				return Renderer.recipe.getCompactRenderedString.bind(Renderer.recipe);
			case UrlUtil.PG_CLASS_SUBCLASS_FEATURES:
				return Renderer.hover.getGenericCompactRenderedString.bind(Renderer.hover);
			case UrlUtil.PG_CREATURE_FEATURES:
				return Renderer.hover.getGenericCompactRenderedString.bind(Renderer.hover);
			case UrlUtil.PG_DECKS:
				return Renderer.deck.getCompactRenderedString.bind(Renderer.deck);
			// region props
			case "classfeature":
			case "classFeature":
				return Renderer.hover.getGenericCompactRenderedString.bind(Renderer.hover);
	...
	static getFnBindListenersCompact(page, { overrides = {} } = {}) {
		...
		switch (page) {
			case UrlUtil.PG_BESTIARY:
				return Renderer.monster.bindListenersCompact.bind(Renderer.monster);
			case UrlUtil.PG_RACES:
				return Renderer.race.bindListenersCompact.bind(Renderer.race);
			default:
				return null;
		}
	}
	...
	static async pDoBrowserPopoutCurPage(evt, entity) {
		...
		await Renderer.hover.pDoShowBrowserWindow($content, {
			title: entity._displayName || entity.name,
		});
	}
};
...
Renderer.findName = function (entry) {
	return CollectionUtil.dfs(entry, { prop: "name" });
};
Renderer.findSource = function (entry) {
	return CollectionUtil.dfs(entry, { prop: "source" });
};
Renderer.findEntry = function (entry) {
	return CollectionUtil.dfs(entry, { fnMatch: (obj) => obj.name && obj?.entries?.length });
};
...
Renderer._stripTags_textRender = function ({ str, ptrAccum, allowlistTags = null, blocklistTags = null } = {}) {
	...
	for (let i = 0; i < len; ++i) {
		...
		if ((allowlistTags != null && allowlistTags.has(tag)) || (blocklistTags != null && !blocklistTags.has(tag))) {
			ptrAccum._ += s;
			continue;
		}
...
Renderer.getRollableRow = function (row, opts) {
	...
	if (row[0]?.type === "cell" && (row[0]?.roll?.exact != null || (row[0]?.roll?.min != null && row[0]?.roll?.max != null))) return row;
